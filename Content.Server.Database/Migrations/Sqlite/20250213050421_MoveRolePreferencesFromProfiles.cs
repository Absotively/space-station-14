...
AS SELECT
  profile.profile_id,
  coalesce(job.job_name, substr(loadout.role_name, 4)) AS job_name,
  CASE WHEN loadout.profile_role_loadout_id IS NOT NULL THEN 3 ELSE min(2, job.priority) END AS priority
